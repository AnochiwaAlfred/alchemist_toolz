...
#                     takes_context=True,
#                     )
# def navigation_inclusion_tag(context, request=None, object=None, menu='menu', permission="admin"):
#     module = importlib.import_module(object)
#     NAV_MAP = getattr(module, "Menus")
#     obj =  NAV_MAP.objects.all().filter(menu_type=menu, permission=permission).order_by("order")
...
#         if not is_allowed:
#             return ""
#         url = f" <a href='/{config('BASE_PATH_NAME')}/{anchor}'><i class='{icon}'></i> {title}</a>"
...
#         if not is_allowed:
#             return ""
#         url = f" <a href='/{config('BASE_PATH_NAME')}/{config('APP_MAIN_NAME')}/menus'><i class='{icon}'></i> {title}</a>"
#         return format_html(url)
#     except:
#         return